...
class Generation(object):
    ...
    def next(self):
        ...
        maximum = max(self.population).f()+1
...
def reproduce(X, Y):
    ...
    while True:
        ...
        # this part is very explicitly coded for efficient calculation of the new tour length
    ...
    for i in range(1, len(tour) - 2):
        for j in range(i+1, len(tour)-1):
            for k in range(j+1, len(tour)):
                ...
                # first try all possible 2-Opt moves in turn
                # this part is very explicitly coded for efficient calculation of the new tour length
                new_tour = s1 + s2 + s3[::-1]
                new_tour_length = base_length + distance_matrix[s1[-1]][s2[0]] + distance_matrix[s2[-1]][s3[-1]] + distance_matrix[s3[0]][s1[0]]
                ...
                new_tour = s1 + s3[::-1] + s2[::-1]
                new_tour_length = base_length + distance_matrix[s1[-1]][s3[-1]] + distance_matrix[s3[0]][s2[-1]] + distance_matrix[s2[0]][s1[0]]
                ...
                new_tour = s1 + s2[::-1] + s3
                new_tour_length = base_length + distance_matrix[s1[-1]][s2[-1]] + distance_matrix[s2[0]][s3[0]] + distance_matrix[s3[-1]][s1[0]]

                if new_tour_length < tour_length:
                    tour = new_tour
                    tour_length = new_tour_length
                                   
                # now try all possible 3-Opt moves in turn
                new_tour = s1 + s3 + s2
