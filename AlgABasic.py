...
added_note = "something about the heuristic I used"
...
# num_cities is the number n

class node(object):
    def __init__(self, _id, state, parent_id, action, path_cost, depth):
        self.id = _id
        self.state = state
    ...
    def pop(self):
        if (not self.isEmpty):
            ...
            for i in range(1,len(self.queue)):
                if heuristic(self.queue[i], self.queue[j]):
                    j = i
            item = self.queue[j]
            del self.queue[j]
            return item
...
class StateSpace(object):
    ...
    def returnById(self, _id):
        
        
                    

def heuristic(objA, objB):
...
def AStarSearch():
    ...
    #(id = some integer, state = some string "0402463" for visited nodes,
    #   parent_id = some integer id, action = weigt of last move, path_cost, depth)
    startNode = node(newid, "0", None, 0,0,0)

    # add the start node to the state space
    stateSpace.add(startNode)

    # init the fringe priority queue
    fringe = PriorityQueue()

    # is the start node a goal node? - not likely
    if isGoalNode(startNode):
        return 1
    else:
        ...
        while fringe != []:
            ...
            for i in range(0,num_cities):
                ...
                weight = distance_matrix[int(node.state[-1])][i]
                
                if weight > 0:
                    # init the child node
                    newid += 1
                    child = node(newid, node.state + str(i), node.id, weight ,node.path_cost + weight ,node.depth + 1)

                    # add the child node to the state space
                    stateSpace.add(child)

                    # if child is a goal node return else continue
                    if isGoalNode(child):
                        return newid
                    else:
                        fringe.push(child)
...
tour = stateSpace.returnById(AStarSearch()).path_cost
